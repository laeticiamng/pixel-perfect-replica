...
export default function DiagnosticsPage() {
  ...
  const refreshLogs = () => {
    setLogs(logger.getRecentLogs(30));
  };
  ...
  const avgLatency = latencyTests.length > 0
    ? Math.round(
        ...
        latencyTests.filter(t => t.latency !== null).length
      )
    : null;
  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-gradient-radial pb-8 safe-bottom">
      ...
      <header className="safe-top px-6 py-4 flex items-center gap-4">
        ...
        <span className="ml-auto text-xs bg-signal-yellow/20 text-signal-yellow px-2 py-1 rounded">
          DEV ONLY
        </span>
      </header>
      ...
      <div className="px-6 space-y-6">
        ...
        <div className="glass rounded-xl p-4">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className={cn(
                'font-semibold',
                isOnline ? 'text-signal-green' : 'text-signal-red'
              )}>
            ...
            <div className="flex justify-between">
              ...
              <span className={cn(
                'font-semibold',
                isWatching ? 'text-signal-green' : 'text-signal-yellow'
              )}>
        ...
        <div className="glass rounded-xl p-4">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              ...
              <span className={cn(
                'font-semibold',
                authLoading ? 'text-signal-yellow' : isAuthenticated ? 'text-signal-green' : 'text-signal-red'
              )}>
        ...
        <div className="glass rounded-xl p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button
              ...
              className="h-8"
            >
              <RefreshCw className={cn("h-4 w-4 mr-1", isTestingLatency && "animate-spin")} />
              Tester
            </Button>
          </div>
            ...
            <div className="space-y-2">
              {latencyTests.map(test => (
                <div key={test.name} className="flex justify-between text-sm">
                  ...
                  <span className={cn(
                    'font-semibold',
                    test.status === 'success' ? 'text-signal-green' : 
                    test.status === 'error' ? 'text-signal-red' : 'text-muted-foreground'
                  )}>
                    {test.status === 'pending' ? '...' :
              ...
              {avgLatency !== null && (
                <div className="flex justify-between text-sm border-t border-border pt-2 mt-2">
                  <span className="text-foreground font-semibold">Moyenne</span>
                  <span className={cn(
                    'font-bold',
                    avgLatency < 200 ? 'text-signal-green' :
                    avgLatency < 500 ? 'text-signal-yellow' : 'text-signal-red'
                  )}>
                    {avgLatency}ms
                  </span>
                </div>
        ...
        <div className="glass rounded-xl p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <Button size="sm" variant="ghost" onClick={refreshLogs} className="h-8">
                <RefreshCw className="h-4 w-4" />
              </Button>
              <Button size="sm" variant="ghost" onClick={clearLogs} className="h-8 text-signal-red">
              ...
              logs.map((log, i) => (
                <div 
                  ...
                  className={cn(
                    ...
                    log.level === 'error' ? 'bg-signal-red/10 text-signal-red' :
                    log.level === 'warn' ? 'bg-signal-yellow/10 text-signal-yellow' :
        ...
        {logger.getRecentErrors(5).length > 0 && (
          <div className="glass rounded-xl p-4 border border-signal-red/20">
            <h2 className="font-semibold text-signal-red mb-4 flex items-center gap-2">
              <AlertTriangle className="h-5 w-5" />
              Erreurs récentes
            </h2>
            
            <div className="space-y-2">
              {logger.getRecentErrors(5).map((log, i) => (
                <div key={i} className="text-xs bg-signal-red/10 p-2 rounded">
                  <p className="text-signal-red font-semibold">{log.message}</p>
                  {log.data && (
                    <p className="text-signal-red/70 mt-1">
