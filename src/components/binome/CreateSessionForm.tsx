...
const formSchema = z.object({
  ...
  duration_minutes: z.number().refine(val => [45, 90, 180].includes(val), {
...
export function CreateSessionForm({ onSubmit, onCancel, isLoading }: CreateSessionFormProps) {
  ...
  const handleSubmit = async (data: FormData) => {
    if (!selectedActivity) {
      form.setError('activity', { message: "Sélectionne une activité" });
      return;
    }
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        {/* Activity Selection */}
        <div>
          <FormLabel className="text-foreground">Activité</FormLabel>
          <div className="mt-2">
            <ActivitySelector
              selectedActivity={selectedActivity}
              onSelect={(activity) => {
                setSelectedActivity(activity as ActivityType);
                form.setValue('activity', activity as ActivityType);
              ...
              <div className="grid grid-cols-3 gap-2">
                {durationOptions.map((option) => (
                  <Button
                    ...
                    className={cn(
                      field.value === option.value && "bg-coral hover:bg-coral/90"
                    )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Lieu précis (optionnel)</FormLabel>
              <FormControl>
                <Input 
                  placeholder="BU Sciences, Café Central..." 
                  {...field} 
                />
              </FormControl>
              <FormMessage />
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((num) => (
                  <Button
                    ...
                    className={cn(
                      "flex-1",
                      field.value === num && "bg-coral hover:bg-coral/90"
                    )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea 
                  placeholder="Prépa ECN, Déjeuner vegan, Running 10km..."
